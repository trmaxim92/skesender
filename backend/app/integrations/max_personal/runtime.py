...
class MaxPersonalRuntime:
    ...
    async def manual_reconnect(self, channel_id: int, *, timeout: float = 45.0) -> None:
        """Force restore from saved session (no QR wipe). Used after auto-reconnect gave up."""
        meta = await self._load_session_meta(channel_id)
        if not meta:
            raise IntegrationError(
                "Нет сохранённой сессии MAX. Подключите канал заново через QR."
            )
        ...
        while asyncio.get_running_loop().time() < deadline:
            ...
            if state is not None and state.status == "error":
                raise IntegrationError(
                    state.error or "Не удалось переподключить MAX · аккаунт"
                )
            await asyncio.sleep(0.25)

        state = self._states.get(channel_id)
        err = (state.error if state else None) or "Таймаут ручного переподключения MAX · аккаунт"
